...
int main(void)
{
	...
	for (int i = 0; i < inputFrame.rows; i++)
	{
		...
		{
			Vec3b point = blur_img.at<Vec3b>(i, j);
	...
	Mat labels;
	int KNum = 2;
    kmeans(data, KNum, labels, TermCriteria(TermCriteria::EPS + TermCriteria::COUNT, 10, 1.0), 1, KMEANS_RANDOM_CENTERS); // TermCriteria迭代条件

    //给每类一个颜色
	Mat kmeans_img = inputFrame.clone();

	//显示聚类结果，不同的类别用不同的颜色显示
	int n = 0;
	for (int i = 0; i < kmeans_img.rows; i++)
	{
		for (int j = 0; j < kmeans_img.cols; j++)
		{
			int clusterIdx = labels.at<int>(n);
			kmeans_img.at<Vec3b>(i, j) = colorTab[clusterIdx];
			n++;
		}
	}
	
	namedWindow("kmeans", WINDOW_NORMAL);
	resizeWindow("kmeans", window_size);
